...
(function() {
  var Backbone, Flow, JsonLoader, LoadAssets, PantaRhei, VERSION, ValueChecker, Worker, root, workers, _,
  ...
  VERSION = PantaRhei.VERSION = "0.2.1";
  ...
  Flow = PantaRhei.Flow = (function() {
    ...
    Flow.prototype.check = function(valuesToTest) {
      var worker;
    ...
    Flow.prototype._cacheFlow = function(worker) {
      return worker._flow = this;
    };
  ...
  ValueChecker = (function(_super) {
    __extends(ValueChecker, _super);

    function ValueChecker(valuesToTest, id) {
      this.valuesToTest = valuesToTest;
      this.id = id != null ? id : _.uniqueId('valueChecker_');
    }

    ValueChecker.prototype.run = function(shared, next) {
      var checkValues;

      checkValues = _.chain(this.valuesToTest).pairs().map(function(el) {
        if (shared[el[0]] === el[1]) {
          return true;
        } else {
          return false;
      ...
      if (_.indexOf(checkValues, false) > -1) {
        return this._flow.terminate();
      } else {
        return next();
      }
    };

    ValueChecker.prototype.kill = function() {};
  ...
  JsonLoader = workers.JsonLoader = (function(_super) {
    ...
    function JsonLoader(jsonUrl, id, randomize) {
      var randomizeQuery;

      this.jsonUrl = jsonUrl;
      this.id = id != null ? id : _.uniqueId('jsonLoader_');
      this.randomize = randomize != null ? randomize : true;
      randomizeQuery = this.randomize ? "?ran=" + (Math.random() * 999999) : '';
      this.jsonUrl = this.jsonUrl + randomizeQuery;
    }
